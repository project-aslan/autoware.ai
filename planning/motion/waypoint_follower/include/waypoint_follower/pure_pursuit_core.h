...
{
public:
  PurePursuitNode(const ros::NodeHandle& nh, const ros::NodeHandle& private_nh);
  ~PurePursuitNode();

  void run();
  double computeLookaheadDistance_test();
  double computeCommandVelocity_test();
  double computeCommandAccel_test();
  double computeAngularGravity_test(double velocity, double kappa);
  void set_param_flag_(int32_t param_flag);
  void set_const_lookahead_distance_(double const_lookahead_distance);
  void set_current_linear_velocity_(double current_linear_velocity);
  void set_command_linear_velocity_(double command_linear_velocity);
  void set_lookahead_distance_ratio_(double lookahead_distance_ratio);
  void set_minimum_lookahead_distance_(double minimum_lookahead_distance);
