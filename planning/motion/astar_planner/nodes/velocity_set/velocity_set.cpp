...
void velocity_set::obstacleColorByKind(const EControl kind, std_msgs::ColorRGBA &color, const double alpha/*=0.5*/)
{
...
void velocity_set::displayObstacle(const EControl& kind, const ObstaclePoints& obstacle_points, const ros::Publisher& obstacle_pub)
{
...
void velocity_set::displayDetectionRange(const aslan_msgs::Lane& lane, const int closest_waypoint,
                           ...
                           const double deceleration_range, const ros::Publisher& detection_range_pub)
{
  ...
  for (int i = 0; i < velocity_set::STOP_SEARCH_DISTANCE; i++)
  {
    ...
    if (i > velocity_set::DECELERATION_SEARCH_DISTANCE)
      continue;
...
int velocity_set::detectStopObstacle(const pcl::PointCloud<pcl::PointXYZ>& points, const int closest_waypoint,
                       ...
                       const int wpidx_detection_result_by_other_nodes)
{
  ...
  *obstacle_type = EObstacleType::NONE;
  ...
  for (int i = closest_waypoint; i < closest_waypoint + velocity_set::STOP_SEARCH_DISTANCE; i++)
  {
...
int velocity_set::detectDecelerateObstacle(const pcl::PointCloud<pcl::PointXYZ>& points, const int closest_waypoint,
                             ...
                             ObstaclePoints* obstacle_points)
{
  ...
  // start search from the closest waypoint
  for (int i = closest_waypoint; i < closest_waypoint + velocity_set::DECELERATION_SEARCH_DISTANCE; i++)
  {
    // reach the end of waypoints
    if (i >= static_cast<int>(lane.waypoints.size()))
      break;
...
EControl velocity_set::pointsDetection(const pcl::PointCloud<pcl::PointXYZ>& points, const int closest_waypoint,
...
EControl velocity_set::obstacleDetection(int closest_waypoint, const aslan_msgs::Lane& lane,
                           ...
                           const ros::Publisher& obstacle_pub, int* obstacle_waypoint)
{
  ...
  if (detection_result == EControl::STOP || detection_result == EControl::STOPLINE || detection_result == EControl::DECELERATE)
  {
    ...
    velocity_set::obstacle_removed_pub.publish(obstacle_removed_flag);
  ...
  if (radar_detection_result == EControl::STOP || radar_detection_result == EControl::STOPLINE || radar_detection_result == EControl::DECELERATE)
  {
      ...
      emergency_stop.data = 1;
      velocity_set::emergency_stop_pub.publish(emergency_stop);
  ...
  if (prev_detection == EControl::STOP || prev_detection == EControl::STOPLINE || prev_detection == EControl::DECELERATE)
  {
    ...
    if (false_count < velocity_set::LOOP_RATE / 2)
    {
...
void velocity_set::changeWaypoints(const VelocitySetInfo& vs_info, const EControl& detection_result, int closest_waypoint,
...
int VELOCITY_SET::RUN(int argc, char** argv)
{
  ...
  velocity_set::emergency_stop_pub = nh.advertise<std_msgs::Int32>("radar_emergency_stop", 100, true);
  ros::Publisher detected_obst_pub = nh.advertise<std_msgs::Bool>("obstacle_detected_stop", 100, true);
  velocity_set::obstacle_removed_pub = nh.advertise<std_msgs::Bool>("obstacle_removed", 100, true);
  ros::Publisher final_waypoints_pub = nh.advertise<aslan_msgs::Lane>("final_waypoints", 100, true);

  ros::Rate loop_rate(velocity_set::LOOP_RATE);
  while (ros::ok())
  {
    ros::spinOnce();
